...
func (m *EndpointsModule) Run(ctx context.Context, domain string) error {
	...
	// Check that at least one input source exists
	subsExists := false
	if _, err := os.Stat(subsFile); err == nil {
		if lines, ok := readLinesOrWarn(subsFile, m.log, "endpoints/subdomains"); ok {
			subsExists = true
			_ = lines
		}
	}
	...
	if _, err := os.Stat(liveURLsFile); err == nil {
		if lines := readLines(liveURLsFile); len(lines) > 0 {
			liveURLsExist = true
		}
	}
	if !subsExists && !liveURLsExist {
		m.log.Warn("  No input files found for endpoint discovery, skipping")
		return nil
	}
	...
	// ── waybackurls (passive — always runs) ──
	if subsExists {
		if _, err := exec.LookPath("waybackurls"); err == nil {
			...
			go func() {
				...
				if output, err := cmd.CombinedOutput(); err != nil {
					m.log.Debug("  waybackurls stderr: %s", string(output))
				}
			...
			m.log.Warn("  waybackurls not found — install: go install github.com/tomnomnom/waybackurls@latest")
		}
	}

	// ── gau (passive — always runs) ──
	if subsExists {
		if _, err := exec.LookPath("gau"); err == nil {
			...
			go func() {
				...
				if output, err := cmd.CombinedOutput(); err != nil {
					m.log.Debug("  gau stderr: %s", string(output))
				}
			...
			m.log.Warn("  gau not found — install: go install github.com/lc/gau/v2/cmd/gau@latest")
		}
	}

	// ── katana (ACTIVE crawling — skip in passive mode) ──
	if !m.cfg.PassiveOnly && liveURLsExist {
		if _, err := exec.LookPath("katana"); err == nil {
			...
			go func() {
				...
				if output, err := cmd.CombinedOutput(); err != nil {
					m.log.Debug("  katana stderr: %s", string(output))
				}
			...
			m.log.Warn("  katana not found — install: go install github.com/projectdiscovery/katana/cmd/katana@latest")
		}
	} else if m.cfg.PassiveOnly {
		m.log.Info("  Skipping katana (passive mode)")
	}

	// ── gospider (ACTIVE crawling — skip in passive mode) ──
	if !m.cfg.PassiveOnly && liveURLsExist {
		if _, err := exec.LookPath("gospider"); err == nil {
			...
			go func() {
				...
				if output, err := cmd.CombinedOutput(); err != nil {
					m.log.Debug("  gospider stderr: %s", string(output))
				}
			...
			m.log.Warn("  gospider not found — install: go install github.com/jaeles-project/gospider@latest")
		}
	} else if m.cfg.PassiveOnly {
		m.log.Info("  Skipping GoSpider (passive mode)")
	}
