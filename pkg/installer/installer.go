	...
	return &Installer{
		...
		tools: []Tool{
			...
			},
			{
				...
				Required: false,
				InstallCmds: []string{
					"sudo apt-get install -y findomain",
			...
			// ── Port Scanning ──
			{
				...
				Binary:   "naabu",
				Category: "ports",
				Required: false, // Changed: not strictly required since nmap can be used as fallback
				InstallCmds: []string{
			...
			},
			{
				...
				Binary:   "nmap",
				Category: "ports",
				Required: true, // Changed: nmap is the universal fallback
				InstallCmds: []string{
...
func (i *Installer) CheckAndInstall() error {
	...
	// Run apt-get update once if we're on a Debian-based system and have sudo
	i.aptUpdateOnce()
	...
	for _, tool := range i.tools {
		...
		for _, cmd := range tool.InstallCmds {
			...
			if err := i.runCommand(cmd); err == nil {
				...
				// Log installation failure reason at Warn level, not just Debug
				i.log.Warn("    Install attempt failed: %s — %v", cmd, err)
			}
		}
...
// aptUpdateOnce runs "sudo apt-get update" once at the start if apt-get is available.
// This ensures package lists are fresh before any apt-based installs are attempted.
func (i *Installer) aptUpdateOnce() {
	...
	if err := i.runCommand("sudo apt-get update -qq"); err != nil {
		i.log.Warn("  apt-get update failed (sudo may not be available): %v", err)
		i.log.Warn("  apt-based installs may fail — consider running as root or pre-installing tools")
	}
}
